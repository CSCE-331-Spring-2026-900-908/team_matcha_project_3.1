      ...
      <section className="mx-auto w-full max-w-7xl">
        <div className="rounded-[32px] border border-[#c8d2c3] bg-white/80 p-8 shadow-[0_24px_70px_rgba(31,37,32,0.08)] backdrop-blur sm:p-10">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-3 self-start sm:self-center">
              <button
                ...
                className="rounded-xl border border-[#2f7a5f] px-6 py-2.5 text-sm font-bold uppercase tracking-wider text-[#2f7a5f] shadow-sm transition-all hover:bg-[#2f7a5f] hover:text-white"
              >
                Switch to Cashier
              </button>
              <button
                onClick={handleLogout}
                className="rounded-xl border border-red-200 bg-red-50 px-6 py-2.5 text-sm font-bold uppercase tracking-wider text-red-700 shadow-sm transition-colors hover:bg-red-100"
              >
                Log Out
              </button>
            </div>
          ...
          <div className="mt-10 grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
            <ManagerSidebar selectedTool={selectedTool} onSelect={setSelectedTool} />
