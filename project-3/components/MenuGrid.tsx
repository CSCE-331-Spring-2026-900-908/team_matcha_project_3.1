  ...
  return (
    <>
      {error ? (
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3" role="list" aria-label="Available drinks">
          {items.map(item => (
            <article
              key={item.menuid}
              role="listitem"
              aria-labelledby={`menu-item-title-${item.menuid}`}
              aria-describedby={`menu-item-description-${item.menuid} menu-item-price-${item.menuid}`}
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-6 text-left">
                <h3 id={`menu-item-title-${item.menuid}`} className="text-[1.45rem] font-bold leading-tight text-[#1f2520]">{item.name}</h3>
                <p id={`menu-item-description-${item.menuid}`} className="mt-3 text-sm leading-6 text-[#5c655c]">
                ...
                <div className="mt-auto flex items-end justify-between gap-4 pt-6">
                  <div>
                    ...
                    <p id={`menu-item-price-${item.menuid}`} className="mt-2 inline-flex rounded-full bg-[#2f7a5f] px-4 py-2 text-xl font-bold text-white shadow-lg shadow-[#2f7a5f]/15">
                  ...
                  <button
                    ...
                    aria-label={`${showAddIcon ? 'Add' : 'Customize'} ${item.name}. Starts at ${currencyFormatter.format(item.cost)}`}
                  >
