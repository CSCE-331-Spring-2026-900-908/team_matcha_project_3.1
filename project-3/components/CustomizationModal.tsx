...
export default function CustomizationModal({
  ...
  if (presentation === 'fullscreen') {
    return (
      <div className="fixed inset-0 z-50 matcha-surface animate-fade-in text-[#1f2520]" role="dialog" aria-modal="true">
            ...
            <main className="flex-1 overflow-y-auto px-8 py-10 sm:px-10 lg:px-16">
              <div className="mx-auto w-full max-w-4xl">
                <header className="border-b border-[#eadfce] pb-8">
                  ...
                  <h2 className="mt-4 text-4xl font-bold text-[#1f2520] sm:text-5xl lg:text-6xl">{item.name}</h2>
                  <div className="mt-5 flex flex-wrap items-center gap-4">
                    ...
                    {toppingCost > 0 ? (
                      <span className="rounded-full border border-[#eadfce] bg-[#f8f1e7] px-5 py-1.5 text-sm font-bold text-[#4a554a]">
                ...
                <div className="mt-10 space-y-8">
                  {CupSizeToggle()}
                  {hotEligible ? TemperatureToggle() : null}
                  ...
                  <section className="rounded-[32px] bg-white p-8 shadow-sm ring-1 ring-[#eadfce]">
                    ...
                    <div className="mt-6 flex flex-wrap gap-4">
                      ...
                      {AVAILABLE_TOPPINGS.map((toppingName) => (
                        <button
                          key={toppingName}
                          onClick={() => toggleTopping(toppingName)}
                          aria-pressed={selectedToppings.includes(toppingName)}
                          className={`min-h-[56px] rounded-full px-8 py-3 text-lg font-bold transition-all focus:outline-none focus:ring-4 focus:ring-[#2f7a5f] focus:ring-offset-2 ${
                            selectedToppings.includes(toppingName) ? 'bg-[#2f7a5f] text-white shadow-xl' : 'bg-[#f8f1e7] text-[#4a554a] hover:bg-[#eadfce]'
                          }`}
                        >
                          {toppingName}
                          {TOPPING_COSTS[toppingName] > 0 ? (
                            <span className="ml-2 opacity-70 font-medium">
                              (+{currencyFormatter.format(TOPPING_COSTS[toppingName])})
                            </span>
                          ) : null}
                        </button>
                      ))}
            ...
            <footer className="border-t border-[#eadfce] bg-white px-8 py-6 shadow-[0_-8px_30px_rgba(47,36,29,0.06)] sm:px-10 lg:px-16">
              <div className="mx-auto flex w-full max-w-4xl items-center justify-between gap-6">
                ...
                <button
                  ...
                  className="min-h-[64px] min-w-[200px] shrink-0 rounded-[24px] bg-[#2f7a5f] px-10 py-5 text-xl font-bold text-white shadow-xl shadow-[#2f7a5f]/20 transition-all hover:bg-[#25634d] active:scale-[0.98] focus:outline-none focus:ring-4 focus:ring-[#2f7a5f] focus:ring-offset-2"
                >
                  {finalConfirmLabel}
                </button>
              </div>
      ...
      <div className="relative w-full max-w-7xl overflow-hidden rounded-[32px] bg-white shadow-2xl animate-scale-in">
        {showDialogImage ? (
          <header className="relative h-56 w-full bg-[#f8f1e7]">
            ...
            <button
              onClick={onClose}
              className="absolute right-6 top-6 flex h-12 w-12 items-center justify-center rounded-full bg-white/90 text-[#1f2520] shadow-md transition-colors hover:bg-white focus:outline-none focus:ring-4 focus:ring-[#2f7a5f]"
              aria-label="Close customization"
            >
          ...
          <button
            onClick={onClose}
            className="absolute right-5 top-5 z-10 flex h-12 w-12 items-center justify-center rounded-full bg-[#f8f1e7] text-[#1f2520] shadow-md transition-colors hover:bg-[#eadfce] focus:outline-none focus:ring-4 focus:ring-[#2f7a5f]"
            aria-label="Close customization"
          >
            <svg width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
              <path d="M18 6L6 18M6 6l12 12" />
        ...
        <main className="p-8">
          <h2 className="pr-14 text-3xl font-bold text-[#1f2520]">{item.name}</h2>
          <div className="mt-2 flex items-center gap-3">
            <p className="text-2xl font-bold text-[#2f7a5f]">{currencyFormatter.format(totalCost)}</p>
            {toppingCost > 0 ? (
              <span className="rounded-full border border-[#eadfce] bg-[#f8f1e7] px-3 py-1 text-sm font-bold text-[#4a554a]">
                Includes {currencyFormatter.format(toppingCost)} add-on
              </span>
            ) : null}
          </div>

          <div className="mt-8 max-h-[50vh] space-y-8 overflow-y-auto pr-2">
            {CupSizeToggle()}
            {hotEligible ? TemperatureToggle() : null}
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                ...
                {AVAILABLE_TOPPINGS.map((toppingName) => (
                  <button
                    ...
                    aria-pressed={selectedToppings.includes(toppingName)}
                    className={`min-h-[48px] rounded-full px-6 py-2 text-base font-bold transition-all focus:outline-none focus:ring-4 focus:ring-[#2f7a5f] ${
                      selectedToppings.includes(toppingName) ? 'bg-[#2f7a5f] text-white shadow-md' : 'bg-[#f8f1e7] text-[#4a554a] hover:bg-[#eadfce]'
                    }`}
                  >
                    {toppingName}
                    {TOPPING_COSTS[toppingName] > 0 ? (
          ...
          <button
            onClick={handleConfirm}
            className="mt-10 min-h-[64px] w-full rounded-[24px] bg-[#2f7a5f] py-5 text-xl font-bold text-white shadow-xl shadow-[#2f7a5f]/20 transition-all hover:bg-[#25634d] active:scale-[0.98] focus:outline-none focus:ring-4 focus:ring-[#2f7a5f] focus:ring-offset-2"
          >
            {finalConfirmLabel}
          </button>
        </main>
